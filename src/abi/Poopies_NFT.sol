...
contract Poopies is ERC721, Ownable {
    using SafeMath for uint256;
    uint public MAX_POOPIES = 10000;
    bool public hasSaleStarted = false;

    constructor(string memory baseURI) ERC721("Poopies", "POOPIES") public { setBaseURI(baseURI); }
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        require(totalSupply() < MAX_POOPIES, "Sale has already ended");
    ...
    function adoptPoopie(uint256 numPoopies) public payable {
        require(totalSupply() < MAX_POOPIES, "Sale has already ended");
        require(numPoopies > 0 && numPoopies <= 20);
        require(totalSupply().add(numPoopies) <= MAX_POOPIES);
        require(msg.value >= calculatePrice().mul(numPoopies), "BNB value sent is below the price");

        for (uint i = 0; i < numPoopies; i++) {
            uint mintIndex = totalSupply();
            _safeMint(msg.sender, mintIndex);
        }
    }

    function mintNFT(uint256 numOf) public payable {
        require(totalSupply() < MAX_POOPIES, "Sale has already ended");
        require(numOf > 0 && numOf <= 20);
        require(totalSupply().add(numOf) <= MAX_POOPIES);
        require(msg.value >= calculatePrice().mul(numOf), "BNB value sent is below the price");
    ...
    // Decrease the MAX_POOPIES variable only if these items aren't sold.
    ...
    function burn(uint256 numPoopiesToBurn) public onlyOwner {
        ...
        uint NewMaxPoopies = MAX_POOPIES - numPoopiesToBurn;
        require(totalSupply() <= NewMaxPoopies, "There NFT's are allready claimed");
        MAX_POOPIES = NewMaxPoopies;
        }
    function setBaseURI(string memory baseURI) public onlyOwner { _setBaseURI(baseURI); }
    function startSale() public onlyOwner { hasSaleStarted = true; }
    function pauseSale() public onlyOwner { hasSaleStarted = false; }
    ...
    function reserveGiveaway(uint256 numPoopies) public onlyOwner {
        ...
        require(totalSupply().add(numPoopies) <= MAX_POOPIES, "Exceeds MAX_POOPIES");
        require(hasSaleStarted == false, "Sale has already started");
        uint256 index;
        for (index = 0; index < numPoopies; index++) {  _safeMint(owner(), currentSupply + index); }
        }
}
