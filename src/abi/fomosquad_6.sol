...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    address public dividendToken = 0x8a9424745056Eb399FD19a0EC26A14316684e274; //testcode
    ...
    function setDividendTokenAddress(address newToken) public { dividendToken = newToken; }
...
contract ShibaFlockiXToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event SendDividends2(uint256 tokensSwapped, uint256 amount);
    ...
    constructor(address [] memory _devs) ERC20("ShibaFlokiX", "SHIBAFLOKIX") {
    	...
    	dividendTracker             = new ShibaFlokiDividendTracker();
        dividendTracker2            = new ShibaFlokiDividendTracker2();
        ...
        dividendTracker2.excludeFromDividends(address(dividendTracker2));
    ...
    receive() external payable {
  	...
  	function whitelistDxSale(address _presaleAddress, address _routerAddress) public onlyOwner {
        ...
        dividendTracker2.excludeFromDividends(_presaleAddress);
        excludeFromFees(_presaleAddress, true);

        dividendTracker.excludeFromDividends(_routerAddress);
        dividendTracker2.excludeFromDividends(_routerAddress);
        excludeFromFees(_routerAddress, true);
  	}
  	function setMaxBuyTransaction(uint256 maxTxn) external onlyOwner { maxBuyTranscationAmount = maxTxn * (10**18); }
  	function setMaxSellTransaction(uint256 maxTxn) external onlyOwner { maxSellTransactionAmount = maxTxn * (10**18); }
  	function setMaxWalletToken(uint256 maxToken) external onlyOwner { maxWalletToken = maxToken * (10**18); }
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "sellFeeIncreaseFactor must be between 100~200");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "marketingDivisor must be between 0~100");
    ...
    function afterPreSale() external onlyOwner {
        setTradingIsEnabled(true); // must be false to handle LP
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "swapping process is currently running, wait until completed");
        
        uint256 buyBackBalance = address(this).balance;
        swapBNBForTokens(buyBackBalance.div(10**2).mul(amount));
    }
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "dividendTracker already has that value");
        ShibaFlokiDividendTracker newDividendTracker = ShibaFlokiDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "dividendTracker must be owned by contract");
    ...
    function updateDividendTracker2(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker2), "dividendTracker2 already has that value");
        ShibaFlokiDividendTracker2 newDividendTracker = ShibaFlokiDividendTracker2(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "dividendTracker2 must be owned by contract");

        newDividendTracker.excludeFromDividends(address(newDividendTracker));
        newDividendTracker.excludeFromDividends(address(this));
        newDividendTracker.excludeFromDividends(address(uniswapV2Router));
        emit UpdateDividendTracker(newAddress, address(dividendTracker2));

        dividendTracker2 = newDividendTracker;
    }
    
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "dividendRewardsFee must be between 0~10");
        dividendRewardsFee = newFee;
        }
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "marketingFee must be between 0~10");
        marketingFee = newFee;
        }
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "uniswapV2Router already has that value");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "_isExcludedFromFees[account] already has that value");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "automatedMarketMakerPairs[pair} already has that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "buybackWallet already has that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "gasForProcessing must be between 200,000~500,000");
        require(newValue != gasForProcessing, "gasForProcessing already has that value");
    ...
    function getAccountDividendsInfo2(address account)
        external view returns ( address, int256, int256, uint256, uint256, uint256, uint256, uint256) {
        return dividendTracker2.getAccount(account);
        }
	function getAccountDividendsInfoAtIndex2(uint256 index) external view returns ( address, int256, int256, uint256, uint256, uint256, uint256, uint256) { 
    ...
    function processDividendTracker2(uint256 gas) external {
		(uint256 iterations, uint256 claims, uint256 lastProcessedIndex) = dividendTracker2.process(gas);
		emit ProcessedDividendTracker2(iterations, claims, lastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function getNumberOfDividendTokenHolders2() external view returns(uint256) { return dividendTracker2.getNumberOfTokenHolders(); }
    /*
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[from]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 swapTokens = 10**20;
                swapTokens = swapTokens.mul(marketingFee).div(totalFees);
                ...
                //else if (buyBackRandomEnabled && buyBackBalance > uint256(5 * 10**18)) {
                //    swapBNBForTokens(buyBackBalance.div(10**3).mul(rand()));
                //}
                
                if(balanceOf(uniswapV2Pair) >= balanceOf(address(this)).mul(10) ) {
                    uint256 sellTokens  = balanceOf(address(this));
                    uint256 sellTokens2 = sellTokens/2; 
                    sellTokens = sellTokens - sellTokens2;
                    if (_dividendToken == uniswapV2Router.WETH()) {
                        swapAndSendDividendsInBNB(sellTokens - sellTokens2);
                    } else {
                        swapAndSendDividends(sellTokens - sellTokens2);
                    }
                    if (_dividendToken2 == uniswapV2Router.WETH()) {
                    swapAndSendDividendsInBNB2(sellTokens2);
                    } else {
                        swapAndSendDividends2(sellTokens2);
                    }
                }
        ...
        if(takeFee) {
            ...
            if(block.timestamp >= launchTime) {
                ...
                //    fees = fees.mul(2); // sell fee 30% first 20 mins after "afterPresale"
                //}
                if(tx.gasprice > 800000000 && block.timestamp - launchTime < 60) {
                    fees = amount.mul(98).div(100); // bot killer
                }
            }
        ...
        try dividendTracker.setBalance(payable(from), balanceOf(from).div(2)) {} catch {}
        try dividendTracker.setBalance(payable(to), balanceOf(to).div(2)) {} catch {}

        try dividendTracker2.setBalance(payable(from), balanceOf(from).div(2)) {} catch {}
        try dividendTracker2.setBalance(payable(to), balanceOf(to).div(2)) {} catch {}
        ...
        if(!swapping) {
	    	...
	    	try dividendTracker2.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit ProcessedDividendTracker2(iterations, claims, lastProcessedIndex, true, gas, tx.origin); } catch { }
        }
    }
    */
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[from]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 swapTokens = (10**20).mul(marketingFee).div(totalFees);
                swapTokensForBNB(swapTokens);
                transferToBuyBackWallet(payable(buyBackWallet), address(this).balance.div(10**2).mul(marketingDivisor));
    ...
    function swapTokensForDividendToken2(uint256 tokenAmount, address recipient) private {
        // generate the uniswap pair path of weth -> busd
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = _dividendToken2;
    ...
    function swapAndSendDividends2(uint256 tokens) private {
        swapTokensForDividendToken2(tokens, address(this));
        uint256 dividends = IERC20(_dividendToken2).balanceOf(address(this));
        bool success = IERC20(_dividendToken2).transfer(address(dividendTracker2), dividends);
        
        if (success) {
            dividendTracker2.distributeDividends(dividends);
            emit SendDividends2(tokens, dividends);
        }
        }
    function swapAndSendDividendsInBNB2(uint256 tokens) private {
        uint256 currentBNBBalance = address(this).balance;
        swapTokensForBNB(tokens);
        uint256 newBNBBalance = address(this).balance;
        
        uint256 dividends = newBNBBalance.sub(currentBNBBalance);
        (bool success,) = address(dividendTracker2).call{value: dividends}("");
...
contract ShibaFlokiDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterableMapping for IterableMapping.Map;

    // address of token game handler rewards
    // spend money on game for "NFT" that returns interest
/*
MainToken: people buy/sell with a 2% Reward fuel tax + 3% marketing tax + %5 buyback
SFX:  Turn off LP/Reward Tax, Marketing 5%
Maybe include a going rate for SFX -> MainToken conversion

400 BNB presale = 120 bnb
WL: Require email & TG & address
500 MainToken "Grab Bag" Doge/Shiba/Eth/BTC/USDT/BUSD/Ping
commonBag = $10 with guaranteed 50 Tokens, 20% chance of winning random token
rareBag = $100 with guaranteed commonBag, 1 raffle token (chance to win 10% of 24h volume), 60% charce of tokens
ultraBag = $? with guaranteed 
...
*/
    ...
    uint256 rareBag;
    uint256 shareSize;
    uint256 halfhourVolume; // if
    uint256 dailyVolume;
    address [] private addressList; // indicates: indexing
    mapping(address => mapping(address => bool)) public hasToken;
    mapping(address => mapping(address =>[uint256, uint256]); // token -> address -> []
    totalShares
    token
    address  
    uSupply // probably not needed
    total
    lastDiv
    lastBuy
    claimed // max = uint128
    ...
    constructor() DividendPayingToken("ShibaFlokiX_Dividend_Tracker", "ShibaFlokiX_Dividend_Tracker") {
    	claimWait = 300; // testcode
    ...
    function withdrawDividend() pure public override { require(false, "withdrawDividend disabled. Use 'claim' function on the main contract");}
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 300 && newClaimWait <= 86400, "claimWait must be between 0.5~24 hours");
        require(newClaimWait != claimWait, "claimWait already has that value");
...
contract ShibaFlokiDividendTracker2 is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterableMapping for IterableMapping.Map;
    ...
    constructor() DividendPayingToken("ShibaFlokiX_Dividend_Tracker2", "ShibaFlokiX_Dividend_Tracker2") {
    	claimWait = 300;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }
