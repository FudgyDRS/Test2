...
library SafeMathInt {
    int256 private constant MIN_INT256 = int256(1) << 255; // 100000000... = -2**255 + 1
    int256 private constant MAX_INT256 = ~(int256(1) << 255); // 01111111... = 2**255 - 1
...
contract DividendDistributor is IDividendDistributor {
    ...
    //TESTCODE
    IBEP20 shibaDividendToken   = IBEP20(0x8a9424745056Eb399FD19a0EC26A14316684e274); // Test Peggy DAI
    //IBEP20 RWRD = IBEP20(0x2170Ed0880ac9A755fd29B2688956BD959F933F8); // ETH/BNB
    //address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; // testcode
    IDEXRouter router;
    ...
    // PSC V2: 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = shibaDividendToken.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(shibaDividendToken);

        router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value}(0, path, address(this), block.timestamp);

        uint256 amount = shibaDividendToken.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            shibaDividendToken.transfer(shareholder, amount);
...
contract DividendDistributor2 is IDividendDistributor {
    ...
    //TESTCODE
    IBEP20 flokiDividendToken   = IBEP20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); // Test Peggy BUSD
    //address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; // testcode
    IDEXRouter router;
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = flokiDividendToken.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = address(flokiDividendToken);

        router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value}(0, path, address(this), block.timestamp);

        uint256 amount = flokiDividendToken.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            totalDistributed = totalDistributed.add(amount);
            flokiDividendToken.transfer(shareholder, amount);
            shareholderClaims[shareholder] = block.timestamp;
            shares[shareholder].totalRealised = shares[shareholder].totalRealised.add(amount);
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);
        }
...
contract ShibaFlokiXToken is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "ShibaFlokiX";
    string constant _symbol = "$SHIBAFLOKIX";
    ...
    uint256 distributor2Gas             = 500000;
    ...
    uint256 public _maxTxAmount                         = rSupply;
    uint256 public _maxWalletToken                      = rSupply;
    ...
    function getTotalDividends() public view returns(uint256, uint256) { return (distributor.totalDistributed(), distributor2.totalDistributed()); }
    //function getDividendsOfHold(address _address) public view returns(uint256, uint256) { return (distributor.getCumulativeDividends(distributor.shares[_address].amount), distributor2.getCumulativeDividends(distributor.shares[_address].amount)); }

    modifier swapping() { inSwap = true; _; inSwap = false; }
    modifier onlyMaster() { require(msg.sender == master || isOwner(msg.sender)); _; }

    event BotKilled(address botAddress, uint256 rentPaid);
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x3002828D4eC5A9b2f3d2Cee65536d5Ad3DF82A07;
        devFeeReceiver       = 0xeeF6d37d3485E77B4dbD552F6a2ac1edA9af1c4B;
    ...
    function afterPresale() external {
        require(authorizations[msg.sender], "Owner only");
        swapEnabled = true;
        swapThreshold = rSupply.div(1000).mul(10);
        !inSwap;
        tradingOpen = true;
        antibotMode = true;
        swapEnabled = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(antibotMode){
            ...
            if(tx.gasprice > maxRoomRent && sender == pair){
                ...
                emit BotKilled(recipient, tx.gasprice);
            }
        }
        ...
        if(!isDividendExempt[sender]) { try distributor2.setShare(sender, balanceOf(sender)) {} catch {} }
        if(!isDividendExempt[recipient]) { try distributor2.setShare(recipient, balanceOf(recipient)) {} catch {} }
        try distributor2.process(distributor2Gas) {} catch {}
    ...
    function checkTxLimit(address sender, uint256 rAmount) internal view { require(rAmount <= _maxTxAmount || isTxLimitExempt[sender], "TX Limit Exceeded"); }
    ...
    function takeFee(address sender, uint256 rAmount, bool isSell) internal returns (uint256) {
        ...
        if(!isSell && (launchedAt + deadBlocks) > block.number){ feeAmount = rAmount.div(100).mul(99); }
        else {feeAmount = rAmount.div(feeDenominator * 100).mul(totalFee).mul(multiplier); }
    ...
    function enable_anti_bot(bool _status) public onlyOwner { antibotMode = _status; }
    function set_max_botTax(uint256 _rent_withoutdecimal) public onlyOwner { maxRoomRent = _rent_withoutdecimal * 10**9; }
    function manage_garbage(address[] calldata addresses, bool status) public onlyOwner { for (uint256 i; i < addresses.length; ++i) { isBot[addresses[i]] = status; } }
    ...
    function swapBack() internal swapping {
        ...
        try distributor2.deposit{value: amountBNBReflection}() {} catch {}
    ...
    function setIsDividendExempt(address holder, bool exempt) external authorized {
        ...
        if(exempt){ distributor2.setShare(holder, 0); }
        else{ distributor2.setShare(holder, balanceOf(holder)); }
    }
    ...
    function rescueToken(address tokenAddress, uint256 tokens) public onlyOwner returns (bool success) { return IBEP20(tokenAddress).transfer(msg.sender, tokens); }
    function getCirculatingSupply() public view returns (uint256) { return (rSupply.sub(_rBalance[DEAD]).sub(_rBalance[ZERO])).div(rate);}
    ...
    function setMaxWalletPercent_base1000(uint256 maxWallPercent_base1000) external onlyOwner() {  _maxWalletToken = rSupply.div(1000).mul(maxWallPercent_base1000); }
    function setMaxTxPercent_base1000(uint256 maxTXPercentage_base1000) external onlyOwner() { _maxTxAmount = rSupply.div(1000).mul(maxTXPercentage_base1000); }
    ...
    function multiTransfer(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
        ...
        for(uint i=0; i < addresses.length; i++){
            ...
            if(!isDividendExempt[addresses[i]]) {
                ...
                try distributor2.setShare(addresses[i], balanceOf(addresses[i])) {} catch {} 
            }
        }
        ...
        if(!isDividendExempt[from]) {
            ...
            try distributor2.setShare(from, balanceOf(from)) {} catch {}
        }
    }
    ...
    function burnRdnmTkn(address _token, address to, uint256 _amount) external { 
        require(authorizations[msg.sender]); //testcode
        IBEP20(_token).transfer(to, _amount); 
    }
