...
contract DogeBack is ERC20, Ownable {
    ...
    address public _devidendToken2 = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    dividendTracker.setDividendTokenAddress(_dividendToken);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if(rand() > 50) {
            dividendTracker.setDividendTokenAddress(_dividendToken);
            _token = _dividendToken;
        } else {
            dividendTracker.setDividendTokenAddress(_dividendToken2);
            _token = _dividendToken2;
        }
        swapTokensForDividendToken(tokens, address(this));
        uint256 dividends = IERC20(_token).balanceOf(address(this));
        bool success = IERC20(_token).transfer(address(dividendTracker), dividends);
