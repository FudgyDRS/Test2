...
describe("transactionsReducer", () => {
  it("addTransaction", () => {
    ...
    expect(
      transactionReducer({}, { type: "ADD_TRANSACTION", payload: transaction })
    ).to.deep.eq({
      1: [transaction],
    });
  });
  ...
  it("correct order", () => {
    ...
    const newState = transactionReducer(
      { 1: [initial] },
      { type: "ADD_TRANSACTION", payload: added }
    );
  ...
  it("update transactions", () => {
    ...
    const newTransactions = initialTransactions.map((tx) => ({
      ...tx,
      lastCheckedBlockNumber: 12,
    }));
